...
Async terminal routes no longer return `202 Accepted` until the proxy has written the OPI request to the configured terminal socket. If a terminal is offline before that point, the HTTP route returns a communication error and any stored transaction is marked `communication_error` instead of remaining `pending`. Black-holed terminal TCP connects now use `OPI_CONNECT_TIMEOUT_MS`, default `3000`.
