...
For printerless standalone terminals, such as EX8000 and A77, the model default is `external` for customer and merchant receipts. That lets the terminal show a customer receipt QR code when enabled by its configuration, or lets the ECR handle receipt output from the proxy callbacks.
