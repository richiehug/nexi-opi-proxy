...
`RESPONSE_MODE=async` returns immediately with HTTP `202`. Payment-style operations include `Location: /transactions/{transactionId}` and:
...
Poll `GET /transactions/:transactionId` for the source-of-truth result. `pending` means the terminal operation is still running. `unknown` means the proxy cannot safely determine the final outcome and the integrator must reconcile operationally.
...
- Interactive payment-style flows use the configured device callback port. Multiple terminals can share the same callback port when they have distinct `workstationId` values. If another interactive request is still active for the same workstation, the proxy responds with HTTP `409` and a `BUSY` error plus the active operation details when available.
