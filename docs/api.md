...
| `GET /terminals/:terminalAlias/ping` | sends four ICMP ping packets to the configured terminal IP and returns only reachability |
...
| `POST /terminals/:terminalAlias/reprint` | asks the terminal to resend the last receipt bundle |
...
`/reprint` remains available, even though it should still be treated as a feature that benefits from further production validation. In practice it is also the main recovery step after `PrintLastTicket` or after a payment whose final response was lost but the terminal completed locally. The proxy always sends `TicketReprint`, including for printerless terminals, because the terminal may still choose to resend receipt output over the callback channel.
...
Synchronous transaction success and error responses expose the transaction details under `transaction`; request metadata such as `requestId`, `retryAfter`, and `error` stay at the response top level. `transaction.amount` is the originally requested amount. When the terminal returns a larger final amount, responses also include `transaction.totalAmount` and `transaction.tip`, where `tip` is the positive difference between `totalAmount` and `amount`. Transaction responses include the configured `transaction.terminalModel` when known. When the terminal reports decline reason `1`, payment errors use public code `AUTHORIZATION_RESPONSE_UNKNOWN`; when it reports `569` or `579`, payment errors use public code `PROVIDER_UNAVAILABLE`; when a force-acceptance payment fails with a terminal "function not supported" prompt, payment errors use public code `FORCE_ACCEPTANCE_UNSUPPORTED`. These responses include vendor-neutral messages and `transaction.failure` with the terminal/host reason fields when available.

For payment transactions, the proxy also returns `transaction.forceAcceptance` when the merchant requested Force Acceptance. If a Force Acceptance payment returns `terminalDeclineReason: "0"`, the proxy returns `transaction.systemAvailable: true`; merchants should then return to normal online mode by sending `forceAcceptance: false` or omitting the parameter on later payments.
...
`RESPONSE_MODE=async` returns HTTP `202` after the proxy has opened the terminal connection, written the OPI request to the configured terminal socket, and waited briefly for an immediate terminal rejection. The grace window is controlled by `ASYNC_ACCEPT_GRACE_MS`, default `1000`; increase it only if you prefer waiting longer to catch slower terminal-side `DeviceUnavailable` or `PrintLastTicket` rejections before returning `202`. If the terminal is offline, the proxy cannot write the request, or the terminal immediately answers with a final failed state such as `DeviceUnavailable` or `PrintLastTicket`, the route returns an error instead of exposing a new `pending` operation. Fresh `DeviceUnavailable` and `PrintLastTicket` responses are cached briefly, controlled by `RETRY_AFTER_DEVICE_UNAVAILABLE_MS`; while the cache is fresh, new operations return `503` plus `Retry-After` instead of creating another pending transaction. The proxy also blocks new payment-style card operations while the terminal still has a lingering `pending` transaction, an unresolved `unknown` transaction, or a most recent `PrintLastTicket` failure that should be cleared by `/reprint` first. Payment-style operations include `Location: /transactions/{transactionId}` and:
...
`/reprint` may automatically reconcile the previous `pending` or `unknown` sale payment to `succeeded` when the reprinted receipt contains strong matching evidence: successful purchase result, approval/reference numbers, matching amount/currency, and receipt time close to the original request. If those checks do not pass, the previous transaction remains `unknown` and should be reconciled with `PATCH /transactions/{transactionId}`. Ambiguous lost-final-response timeouts also try this reprint recovery automatically before leaving a transaction unresolved.
...
Poll `GET /transactions/:transactionId` for the source-of-truth result. `pending` means the OPI request was handed to the terminal and the terminal operation is still running. `communication_error` means the proxy could not reach the terminal before sending the OPI request. `failed` means the terminal returned a final failed outcome, including terminal-level timeouts that contain no approval or transaction reference; Q30/CCV `TimedOut` responses with only generic `OTHR` circuit data; or a lost final response after clear terminal display prompts for unsupported/declined card, invalid card, card-read failure, or terminal timeout. `aborted` means the terminal or cardholder cancelled, including lost-final-response cases where the captured display prompts clearly indicate cancellation or customer/merchant stop. `unknown` means the request may have reached the terminal, but the proxy cannot safely determine the final outcome and the integrator must reconcile operationally.
...
- `POST /terminals/:terminalAlias/activate` and `/deactivate` normally return early when the proxy cache already says the terminal is active or inactive. Send `{ "force": true }` to bypass the cached state. The proxy now tries `ActivateTerminal` / `DeactivateTerminal` first and falls back to `Login` / `LogOff` for terminal variants that still expect the legacy request type. If the terminal still only answers `DeviceUnavailable` or `PrintLastTicket`, `force=true` also permits a local proxy-state override so the cached state does not stay wedged forever.
...
- `POST /terminals/:terminalAlias/abort` forwards OPI `AbortRequest`. If the terminal no longer has an active flow to cancel, the terminal may answer with `Failure`. If a Q30 answers `DeviceUnavailable` while the original card transaction is still pending, the proxy returns `202` with `abortAccepted=false` and `activeTransactionId`; continue polling that transaction for the final result.
