...
**Enhanced Payment Response, DCC Handling, Terminal Operations, Many Small Fixes!**

This release adds structured DCC results, fixes terminal receipt overrides, and makes abort, reversal, and `PrintLastTicket` recovery safer and clearer.

### Highlights

- synchronous payment and transaction-read responses include `transaction.dcc` when DCC was offered, with the cardholder's choice, offered billing amount/currency, exchange rate, and markup when supplied
- terminal-level `receiptHandling` now correctly overrides profile/model defaults; external copies are stored in JSON or SQLite according to `STORAGE`
- immediate `PrintLastTicket` returns `PRINT_LAST_TICKET` instead of `202 Accepted`, while raw terminal results remain separate from cached recovery interpretation
- standalone `/abort` calls return `NO_ACTIVE_TRANSACTION` without contacting the terminal; confirmed aborts return the affected original transaction as `aborted`
- reversal moves the original transaction through `reversing` to `reversed`, restores its previous status on explicit failure, and uses `unknown` for uncertain outcomes
- default `forceAcceptance: false` and redundant conclusive `overallResult` fields are omitted from transaction responses
- normal logs show only the sanitized merchant-submitted payment body; extended logs separate resolved defaults from the effective OPI request
- HTTP and OPI traces share redaction for PAN, expiry, track data, verification codes, bearer data, and raw XML sections
- public and internal documentation now follows numbered reading paths, with clearer API overviews, complete environment-option guidance, and easier-to-scan configuration, storage, security, and upgrade references

### Integrator Notes

- `transaction.dcc` is omitted when DCC was not offered; `accepted=false` means the cardholder chose the transaction currency.
- `202 Accepted` means the terminal operation is pending, not successful. Continue polling the transaction URL.
