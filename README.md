...
This release bundle is for `1.0.5`.
...
5. Choose how to get the image:
   `docker compose pull` to fetch `ghcr.io/richiehug/nexi-opi-proxy:1.0.5` from GHCR without a GitHub login
   or `docker load -i image/nexi-opi-proxy-1.0.5-linux-arm64.tar` on Raspberry Pi if the matching public release asset was provided separately
